...
import AuthoritySubjectForm from "@/components/authority-subject-form-page/authority-subject-form";
import { mapAuthorityDetailToSubjectFormValues } from "@/components/authority-subject-form-page/subject-form.mapper";
import MarcEditor, {
...
export default function AuthoritySubjectFormPage({
  ...
  const saveMutation = useMutation({
    mutationFn: (saveData: MarcEditorSaveData) => {
      ...
      if (!currentRecordKey) {
        throw new Error("수정할 주제명 전거 레코드 키가 없습니다.");
      }
  ...
  const initialFormValues = useMemo(
    () =>
      authorityDetail
        ? mapAuthorityDetailToSubjectFormValues(authorityDetail.data)
        : undefined,
    [authorityDetail],
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="row">
          <AuthoritySubjectForm initialValues={initialFormValues} />
...
function getFallbackSaveError(isCreatePage: boolean): MarcEditorSaveError {
  return {
    ...
    message: isCreatePage
      ? "주제명 전거를 저장하지 못했습니다. 잠시 후 다시 시도해 주세요."
      : "주제명 전거를 수정하지 못했습니다. 잠시 후 다시 시도해 주세요.",
    details: [],
  };
}
