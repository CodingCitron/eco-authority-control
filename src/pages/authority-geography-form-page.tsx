...
import AuthorityGeographyForm from "@/components/authority-geography-form-page/authority-geography-form";
import { mapAuthorityDetailToGeographyFormValues } from "@/components/authority-geography-form-page/geography-form.mapper";
import MarcEditor, {
...
export type AuthorityGeographyFormMode = "create" | "edit";

export default function AuthorityGeographyFormPage({
  mode,
}: {
  mode: AuthorityGeographyFormMode;
}) {
  ...
  const saveMutation = useMutation({
    mutationFn: (saveData: MarcEditorSaveData) => {
      ...
      if (!currentRecordKey) {
        throw new Error("수정할 지리명 전거 레코드 키가 없습니다.");
      }
    ...
    onSuccess: async (data: AuthorityDetailResponse) => {
      ...
      if (isCreatePage) {
        window.alert("지리명 전거가 생성되었습니다.");
        navigate(
          {
            pathname: "/geography/edit",
            search: createSearchParams({ recKey: savedRecordKey }).toString(),
          },
          { replace: true },
        );
        return;
  ...
  const initialFormValues = useMemo(
    () =>
      authorityDetail
        ? mapAuthorityDetailToGeographyFormValues(authorityDetail.data)
        : undefined,
    [authorityDetail],
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="row">
          <AuthorityGeographyForm initialValues={initialFormValues} />
...
function getFallbackSaveError(isCreatePage: boolean): MarcEditorSaveError {
  return {
    ...
    message: isCreatePage
      ? "지리명 전거를 저장하지 못했습니다. 잠시 후 다시 시도해 주세요."
      : "지리명 전거를 수정하지 못했습니다. 잠시 후 다시 시도해 주세요.",
    details: [],
  };
}
